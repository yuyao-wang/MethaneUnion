...
class BasicBlockWithParallelBranches(nn.Module):
    def __init__(self, in_planes, planes, stride=1, downsample=None, reduction = 16):
        ...
        self.se_spatial = SEBlock(planes, reduction) 
        self.se_frequency = SEBlock(planes, reduction) 
    ...
    def forward(self, x):
        ...
        out = self.se_spatial(conv_out) + self.se_frequency(fourier_out)
...
class FreCAResNet(nn.Module):
    def __init__(self, block, layers, input_channel = 18, num_classes=1000, reduction = 16):
        super(FreCAResNet, self).__init__()
        self.in_planes = 64
...
def frequency_ca_resnet18(input_channel = 18, num_classes=1000):
    return FreCAResNet(BasicBlockWithParallelBranches, [2, 2, 2, 2], input_channel, num_classes)