...
flags.DEFINE_string('config_file', '',
                    'Optional path to a JSON file overriding these flags.')
...
flags.DEFINE_string(
    'data_dir', 'data/pretrain/data_download_chips_32',
    'Directory containing methane chip TIFF files.')
flags.DEFINE_string(
    'stats_path', '',
    'Optional path to a cached .npz file with per-band mean/std.')
flags.DEFINE_integer('image_size', 128, 'Spatial resolution after upsampling.')
flags.DEFINE_integer('num_channels', 12, 'Number of spectral bands per chip.')
...
def _apply_config_file():
  ...
  for key, value in config_data.items():
    if key not in FLAGS:
      raise ValueError(f'Unknown flag "{key}" in config file {config_path}')
...
def main(_):
  ...
  if FLAGS.experiment_mode == 'pretrain':
    ...
    if FLAGS.stats_path:
      dataset_config['stats_path'] = FLAGS.stats_path
    config = byol_config.get_config(
        FLAGS.pretrain_epochs,
        FLAGS.batch_size,
    ...
    config = eval_config.get_config(
        f'{FLAGS.checkpoint_root}/pretrain.pkl',
        FLAGS.batch_size,
        dataset_config=dataset_config)
  else:
